(ns respondent.core
  ...
  #+clj
  (:import [clojure.lang IDe])
  #+clj
  (:require [clojure.core.async :as async :refer [go go-loop chan <! >! timeout map> filter> close! mult tap untap]])
  #+cljs
  (:require [cljs.core.async :as async
             :refer [chan <! >! timeout map> filter> close! mult tap untap]])
  (:require-macros [respondent.core :refer [behavior]]
                   [cljs.core.async.macros :refer [go go-loop]])
  (:import (respondent.core IObservable))
  (:import (respondent.core IToken))
  (:import (clojure.lang IDeref)))

(defprotocol IBehavior
  (sample [b interval]
    "Turns this Behavior into EventStream from the sampled values at the given interval"))
...
(defprotocol IToken
  (dispose [tk]
    "Called when the subscriber inst't interested in receiving more items"))
...
;Do the exercises later using clc